...
# Описываем схему прямо здесь, чтобы избежать циклических импортов
...
class LLMAnalyzer:
    def __init__(self):
        ...
        # Берем данные строго из провалидированного Pydantic-конфига
        self.iam_token = settings.YANDEX_IAM_TOKEN
        self.folder_id = settings.YANDEX_FOLDER_ID

    def analyze_text(self, transcript_text: str) -> dict:
        if not transcript_text:
            return {"error": "Текст пустой"}

        print("🧠 Отправка транскрипта в настоящую YandexGPT для бизнес-анализа...")

        # Системный промпт, который заставляет модель вернуть строго JSON по нашей схеме
        prompt = {
        ...
        try:
            response = httpx.post(self.api_url, json=prompt, headers=headers, timeout=30.0)
            ...
            # Очищаем от возможных markdown-обёрток ```json ... ```, если модель их вернёт
            raw_json = raw_json.strip().strip("`").replace("json", "").strip()
            
            # Десериализуем и валидируем через Pydantic (для подстраховки)
